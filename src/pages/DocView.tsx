...
export default function DocView() {
  ...
  if (!doc) return <div className="p-20 text-center font-bold text-slate-300">Carregando...</div>;
    ...
    <div className="bg-slate-50 min-h-screen pb-20 font-sans">
      ...
      <div className="max-w-4xl mx-auto p-12 bg-white mt-10 shadow-xl rounded-2xl min-h-screen text-slate-700">
        <header className="border-b-4 border-blue-600 pb-10 mb-16">
          ...
          <p className="text-slate-300 font-mono text-[10px] mt-2 italic">v.{doc.version}</p>
        </header>

        {/* SEÇÕES FIXAS: INTRODUÇÃO E BENEFÍCIOS (PODE SER IGUAL AO ANTERIOR) */}
        ...
        {doc.sections.map((sec, idx) => {
          ...
          return (
            <div key={sec.id} className="mb-24 print:break-before-page">
              <h2 className="text-3xl font-bold text-slate-800 mb-6 flex items-center gap-3 border-b-2 border-slate-50 pb-2">
              ...
              {/* SEÇÃO ESPECIAL: CONTATOS LADO A LADO */}
              {sec.title === 'Iniciando a integração' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                  ...
                  <div className="p-8 border rounded-2xl bg-white shadow-sm border-t-4 border-t-blue-600">
                    ...
                    <div className="space-y-3 text-sm text-slate-500">
                      <div className="flex items-center gap-2 font-medium">👤 Marcela Espindola</div>
                      <div className="flex items-center gap-2 font-medium">✉️ integrations@audaces.com</div>
                    </div>
                  </div>
                </div>
              )}
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {sec.fields.map((f, i) => (
                        <tr key={i}>
                          ...
                          <td className="p-4 text-center">{f.required ? <span className="text-red-500 font-black">SIM</span> : 'NÃO'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
