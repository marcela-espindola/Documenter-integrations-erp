      ...
      {sections.map((sec) => (
        <div key={sec.id} className="bg-white p-8 rounded-xl shadow-sm border mb-10 border-l-4 border-l-blue-500">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
             <select className="p-2 border text-sm" onChange={e => updateSection(sec.id, 'noteType', e.target.value)}>
                <option value="info">Info</option><option value="warning">Aviso</option><option value="success">Sucesso</option>
             </select>
             <input placeholder="Texto da Nota..." className="p-2 border text-sm" onChange={e => updateSection(sec.id, 'noteContent', e.target.value)} />
          </div>
          ...
          <div className="flex gap-2 flex-wrap mt-2">
            {sec.images.map((img, i) => <img key={i} src={img} className="w-16 h-16 object-cover border" />)}
          </div>
        </div>
      ))}
